...
public OnDemoDecompressed(BZ_Error:iError, String:inFile[], String:outFile[], any:pack) {
	ResetPack(pack);
	char filename[500], path[PLATFORM_MAX_PATH];
	int start_tick;

	ReadPackString(pack, filename, sizeof(filename));
	start_tick = ReadPackCell(pack) - 100;
	CloseHandle(pack);
	PrintToChatAll("START IS %d", start_tick);

	if(_:iError < 0) {
		PrintToAdmins("{red}Failed {white}Decompressing %s.bz2...", filename);

		return;
	}
	PrintToAdmins("{lightskyblue}Decompressed");

	BuildPath(Path_SM, path, sizeof(path), "recordings/played/%s", filename);
	PlayDemo(path, start_tick);
}

public PlayDemo( char[] path, int start_tick)
{
	ServerCommand("playdemo %s;", path);
	return;
}
