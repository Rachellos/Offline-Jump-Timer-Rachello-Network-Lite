...
stock void DB_InitializeDatabase()
{
	...
	g_hDatabase.Query( Threaded_Empty,
		"CREATE TABLE IF NOT EXISTS `mapcprecs` (\
		  ...
		  primary key (map, id, run, mode, uid)\
		)");
...
stock void DB_Profile( int client, int args, int how, char[] Name, int id, int mode = MODE_SOLDIER )
{
	...
	g_hDatabase.Format( szQuery, sizeof( szQuery ),  "(SELECT SUM(pts) FROM maprecs WHERE uid = %i), \
		(SELECT SUM(pts) FROM maprecs WHERE uid = %i and mode = %i), \
		(SELECT SUM(pts) FROM maprecs WHERE uid = %i AND `rank` = 1 and mode = %i), \
		(SELECT SUM(pts) FROM maprecs WHERE uid = %i AND `rank` > 1 AND `rank` < 11 and mode = %i), \
		(SELECT COUNT(overall) FROM plydata WHERE overall > 0.1), \
		(SELECT COUNT(%s) FROM plydata WHERE %s > 0.1), \
		(SELECT COUNT(map) FROM maprecs WHERE uid = %i AND mode = %i and run = 0), \
		(SELECT COUNT(map) FROM mapbounds WHERE (zone = 0 or zone = 2) and number = 0 and (select %s from map_info where map_name = mapbounds.map and run = 0 limit 1) > 0)", 
		id, id, mode, id, mode, id, mode, (mode == MODE_SOLDIER) ? "solly" : "demo", (mode == MODE_SOLDIER) ? "solly" : "demo", id, mode,
		(mode == MODE_SOLDIER) ? "stier" : "dtier" );


		g_hDatabase.Format( szQuery, sizeof( szQuery ),  "SELECT name, uid, orank, %s, online, (SELECT %i), %s, (SELECT %i limit 1) FROM plydata WHERE uid = %i", (mode == MODE_SOLDIER) ? "srank" : "drank", id, szQuery, mode, id );
...
public int Handler_Prifile( Menu mMenu, MenuAction action, int client, int item )
{
	...
	if (item == 4)
	{
		DB_Completions(client, db_id[client], profile_mode[client]);
	}
	if ( item == 5 )
	{
		char szQuery[192];
...
public void Threaded_DemoInfo( Database hOwner, DBResultSet hQuery, const char[] szError, int client )
{
	...
	if ( hQuery.FetchRow() )
	{
		...
		{
			panel.CurrentKey = 3;
			DrawPanelItem(panel,"Play demo (closes current session)");
...
public int demo_control(Menu mMenu, MenuAction action, int client, int item) {
	if (action == MenuAction_Select) {
		...
		{
			char path[PLATFORM_MAX_PATH], out[PLATFORM_MAX_PATH], filename[500];
			Handle pack = CreateDataPack();

			StrCopy(filename, sizeof(filename), DemoUrlClient[client]);

			ReplaceString(filename, sizeof(filename), ".bz2", "");
			WritePackString(pack, filename);
			WritePackCell(pack, g_startTickForDemoPlay);

			BuildPath(Path_SM, path, sizeof(path), "recordings/bz2/%s", DemoUrlClient[client]);

			BuildPath(Path_SM, out, sizeof(out), "recordings/played");

			if(!DirExists(out)) {
				CreateDirectory(out, 0o777);
			}
			BuildPath(Path_SM, out, sizeof(out), "recordings/played/%s", filename);
			if (FileExists(out))
			{
				PlayDemo(out, g_startTickForDemoPlay);
				return 0;
			}

			CPrintToChat(client, CHAT_PREFIX..."Decompression %s...", DemoUrlClient[client]);

			BZ2_DecompressFile(path, out, OnDemoDecompressed, pack);
		}
		else if (item == 4)
		{
			char demosz[150];
...
stock bool DB_SaveClientRecord( int client, float flNewTime )
{
	...
	flPrevMapBest = g_flMapBestTime[run][mode];
	...
	{
		g_flMapBestTime[run][mode] = g_flClientBestTime[client][run][mode];
...
stock void DB_DeleteRecord( int client, int run, int mode, int uid, char[] map )
{
	...
	FormatEx( szQuery, sizeof( szQuery ), "SELECT uid, run, mode, time, name FROM maprecs natural join plydata WHERE map = '%s' order by time asc", g_szCurrentMap );
