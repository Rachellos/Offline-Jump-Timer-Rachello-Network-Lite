...
public void Threaded_Completions( Database hOwner, DBResultSet hQuery, const char[] szError, int client )
{
	...
	if ( hQuery.RowCount )
	{
		while ( hQuery.FetchRow() )
		{
			...
			else
			{
				...
				mMenu.AddItem("c", (profile_mode[client] == MODE_SOLDIER) ? "[Soldier]" : "[Demoman]");
	...
	mMenu.ExitBackButton = true;
	mMenu.SetTitle( "<Completions menu :: %s>\nPlayer: %s :: (%i total)\n ", g_szModeName[NAME_LONG][style], DBS_Name[client], num );	
	SetNewPrevMenu(client, mMenu);
	mMenu.Display( client, MENU_TIME_FOREVER );
	...
	if ( (client = GetClientOfUserId( hData.Get( 0, 0 ) )) )
	{
		...
		if( rank != oldrank )
		{
			...
			if (rank < 11 || szOldTimePts[client][run][mode] <= TIME_INVALID)
			{
				...
				g_hDatabase.Format(szTrans, sizeof(szTrans), "update maprecs SET `rank` = 1 WHERE map = '%s' AND run = %i AND mode = %i", g_szCurrentMap, run, mode );
				transaction.AddQuery(szTrans);
			...
			g_hDatabase.Format(szTrans, sizeof(szTrans), "UPDATE "...TABLE_PLYDATA..." SET %s = 1 where %s > 0.0", (mode == MODE_SOLDIER) ? "srank" : "drank", (mode == MODE_SOLDIER) ? "solly" : "demo" );
			transaction.AddQuery(szTrans);

			g_hDatabase.Format(szTrans, sizeof(szTrans), "UPDATE "...TABLE_PLYDATA..." SET orank = 1 where solly > 0.0 or demo > 0.0" );
			transaction.AddQuery(szTrans);
...
public void Threaded_Init_Records( Database hOwner, DBResultSet hQuery, const char[] szError, any data )
{
	...
	if ( hQuery.RowCount )
	{
		...
		if ( hQuery.FetchRow() )
		{
			...
			iMode = hQuery.FetchInt( 2 );
			
			g_flMapBestTime[iRun][iMode] = hQuery.FetchFloat( 3 );

			hQuery.FetchString( 4, szWrName[iRun][iMode], sizeof(szWrName) );
		}
	}
	else
	{
		for (int run = 0; run < NUM_RUNS; run++)
			for (int mode = 0; mode < NUM_MODES; mode++)
				g_flMapBestTime[run][mode] = TIME_INVALID;
	}
}
