...
stock void ShowKeyHintText( int client, int target )
{
	...
	if ( g_flMapBestTime[run][mode] != TIME_INVALID )
	{
		FormatSeconds( g_flMapBestTime[run][mode], szBestTime, FORMAT_3DECI );
	...
	if ( g_flClientBestTime[target][run][mode] <= g_flMapBestTime[run][mode] || g_flClientBestTime[target][run][mode] <= TIME_INVALID)
	{
		FormatEx( szTxt, sizeof( szTxt ), "");
	}
	else if ( g_flClientBestTime[target][run][mode] > g_flMapBestTime[run][mode])
	{
		float interval = g_flClientBestTime[target][run][mode] - g_flMapBestTime[run][mode];
