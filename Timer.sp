...
float g_flMapBestTime[NUM_RUNS][NUM_MODES];
...
public void DB_Completions( int client, int uid, int mode)
{
	char szQuery[192];
	profile_mode[client] = mode;
	FormatEx( szQuery, sizeof( szQuery ), "SELECT map, recordid, style FROM "...TABLE_RECORDS..." WHERE uid = %i AND run = 0 AND mode = %i",
	uid,
	mode );
...
{
	char query[800];
	g_hDatabase.Format(query, sizeof(query), "SELECT id, time, (select time from maprecs where uid = %i and map = '%s' and run = 0 and mode = %i) FROM "...TABLE_CP_RECORDS..." WHERE map = '%s' and mode = %i and uid = %i ORDER BY id ASC", uid, map, mode, map, mode, uid);
...
public void Threaded_Checkpoint_Times( Database hOwner, DBResultSet hQuery, const char[] szError, int client )
{
	...
	if (hQuery.RowCount)
	{
		if ( hQuery.FetchRow() )
		{
			...
			float time = hQuery.FetchFloat( 1 );
			float prtime = hQuery.FetchFloat( 2 );

			FormatSeconds( prtime, szTimeMap, FORMAT_2DECI );

			FormatEx(buffermap,sizeof(buffermap),"Total:          %s\n ", szTimeMap);
			
			FormatSeconds( time, szTime, FORMAT_2DECI );

			FormatEx(buffer,sizeof(buffer),"[CP %i]:        %s", id, szTime);
		...
		while ( hQuery.FetchRow() )
		{
			...
			if (count == 6)
			{
				FormatEx(buffer,sizeof(buffer),"[CP %i]:        %s\n", id, szTime);
				mMenu.AddItem("", buffer, ITEMDRAW_DISABLED );
